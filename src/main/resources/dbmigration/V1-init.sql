...
(
    ...
    `start_book_info`     text       NOT NULL COMMENT '用户名',
    `cookie_valid_status` tinyint(1) NOT NULL COMMENT '订票信息',
    `schedule_time`       datetime   NOT NULL COMMENT '调度时间',
    `run_status`          tinyint(1) NOT NULL COMMENT '运行状态',
    `book_type`           tinyint    NOT NULL COMMENT '抢订模式',
    `user_agent_index`    tinyint    NOT NULL COMMENT 'agent索引',
